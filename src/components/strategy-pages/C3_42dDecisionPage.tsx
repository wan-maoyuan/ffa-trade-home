...
const C3_42dDecisionPage: React.FC = () => {
  const navigate = useNavigate()
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [analysis, setAnalysis] = useState<C3Analysis | null>(null)
      ...
      try {
        ...
        if (result.code === 200 && result.data.records && result.data.records.length > 0) {
          ...
          if (rawTableData && Array.isArray(rawTableData)) {
            // 解析基础信息
            // Row 1: ["做多胜率统计","盈亏比：","3.43：1"]
            // Row 2: ["做多","2025-11-26","24.69","6%"]
            // Row 3: ["日期","当期值","综合价差比"]
            // Row 4: ["建议交易方向","0%-15%","26.21","19%"]
            // Row 5: ["综合价差比区间","2026-01-07预测值","在全部交易日期中出现概率"]
            
            let profitLossRatio = 3.43
            ...
            // 解析C3盈亏比数据
            // 查找"C3盈亏比"行
            // Row 28: ['C3盈亏比'] (在rawTableData中的实际索引)
            // Row 29: ['2025-11-26', '24.69', '24.59', '0%'] - 数据行（日期、当前价格、评估价格、价差比）
            // Row 30: ['日期', '当前价格/元每吨', '评估价格/元每吨', '价差比'] - 标题行
            // Row 31: ['-0.15~0'] - 区间（可忽略）
            // Row 32: ['63%', '12%', '37%', '6%'] - 数据行（42天后盈利比例、收益均值、42天后亏损比例、亏损均值）
            // Row 33: ['42天后盈利比例', '收益均值', '42天后亏损比例', '亏损均值/元每吨'] - 标题行
            // Row 34: ['最大收益时间在各时间段的出现概率'] - 标题
            // Row 35: ['0～14天', '15天～28天', '29天～42天'] - 标题行
            // Row 36: ['45%', '27%', '28%'] - 最大收益时间数据
            // Row 37: ['-7%', '-29%'] - 最大风险均值、最大风险极值
            // Row 38: ['最大风险均值/元每吨', '最大风险极值/元每吨'] - 标题行
            // Row 39: ['最大风险时间在各时间段的出现概率'] - 标题
            // Row 40: ['0～14天', '15天～28天', '29天～42天'] - 标题行
            // Row 41: ['42%', '29%', '29%'] - 最大风险时间数据
            let c3ProfitLossData: C3ProfitLossRatio | null = null
            for (let i = 0; i < rawTableData.length; i++) {
              const row = rawTableData[i]
              if (Array.isArray(row) && row.length > 0 && row[0] === 'C3盈亏比') {
                // Row 2: 数据行（日期、当前价格、评估价格、价差比）
                if (i + 1 < rawTableData.length) {
                  const dataRow = rawTableData[i + 1]
                  if (Array.isArray(dataRow) && dataRow.length >= 4) {
                    const c3Date = String(dataRow[0] || '')
                    const c3CurrentPrice = parseFloat(String(dataRow[1] || '0').replace(/,/g, '')) || 0
                    const c3EvaluatedPrice = parseFloat(String(dataRow[2] || '0').replace(/,/g, '')) || 0
                    const c3PriceDiffRatio = String(dataRow[3] || '')
                    ...
                    c3ProfitLossData = {
                      date: c3Date,
                      current_price: c3CurrentPrice,
                      evaluated_price: c3EvaluatedPrice,
                      price_difference_ratio: c3PriceDiffRatio,
            ...
            parsedData = {
              ...
              c3_profit_loss_ratio: c3ProfitLossData || {
                date: date || result.data.date,
          ...
          if (c3Analysis) {
            ...
            // 如果c3_analysis不存在，使用core_data构造基本数据
  ...
  return (
    <div className="c3-42d-decision-panel">
      <SideMenu currentPage="strategy" />
      
      <div className="c3-42d-decision-container">
        {/* 返回按钮 */}
        <button 
          className="c3-42d-decision-back-button"
          onClick={handleBackClick}
          type="button"
          aria-label="返回上一级"
        >
          <span className="c3-42d-decision-back-icon">←</span>
          <span className="c3-42d-decision-back-text">返回</span>
        </button>

        <div className="c3-42d-decision-page">
          {/* 页面标题 */}
          <p className="c3-42d-decision-page-title">C3现货应用决策（42天后）</p>

          {loading ? (
            <div className="c3-42d-decision-loading">
              <div className="c3-42d-decision-loading-spinner"></div>
              <p>加载中...</p>
            </div>
          ) : error ? (
            <div className="c3-42d-decision-error">
              <p>{error}</p>
            </div>
          ) : analysis ? (
            <>
              {/* 做多胜率统计 */}
              <div className="c3-42d-decision-header">
                <div className="c3-42d-decision-chips">
                  <button type="button" className="c3-42d-decision-chip">做多胜率统计</button>
                  <button type="button" className="c3-42d-decision-chip">
              ...
              {/* 主要内容区域 */}
              <div className="c3-42d-decision-content">
                {/* 左侧策略卡片和右侧指标网格 */}
                <div className="c3-42d-decision-cards-container">
                  {/* 左侧策略卡片 */}
                  <div className="c3-42d-decision-card-left">
                    <span className="c3-42d-decision-card-badge">多头策略</span>
                    <div className="c3-42d-decision-card-glow" />
                    <div className="c3-42d-decision-card-overlay">
                      <div className="c3-42d-decision-card-title">
                        {analysis.trading_recommendation.recommended_direction}
                      </div>
                      <div className="c3-42d-decision-card-desc">建议交易方向</div>
                    </div>
                  </div>

                  {/* 右侧指标网格 */}
                  <div className="c3-42d-decision-metrics-grid">
                    <div className="c3-42d-decision-metric-card">
                      <p className="c3-42d-decision-metric-label">日期</p>
                      <p className="c3-42d-decision-metric-value">{analysis.current_forecast.date}</p>
                    </div>
                    <div className="c3-42d-decision-metric-card">
                      <p className="c3-42d-decision-metric-label">当期值</p>
                      <p className="c3-42d-decision-metric-value">
                        {analysis.current_forecast.current_value.toLocaleString()}
                      </p>
                    </div>
                    <div className="c3-42d-decision-metric-card">
                      <p className="c3-42d-decision-metric-label">综合价差比</p>
                      <p className="c3-42d-decision-metric-value">{analysis.current_forecast.overall_price_difference_ratio}</p>
                    </div>
                    <div className="c3-42d-decision-metric-card">
                      <p className="c3-42d-decision-metric-label">综合价差比区间</p>
                      <p className="c3-42d-decision-metric-value">{analysis.current_forecast.overall_price_difference_range}</p>
                    </div>
                    <div className="c3-42d-decision-metric-card">
                      <p className="c3-42d-decision-metric-label">2026-01-06预测值</p>
                      <p className="c3-42d-decision-metric-value">
                        {analysis.current_forecast.forecast_value.toLocaleString()}
                      </p>
                    </div>
                    <div className="c3-42d-decision-metric-card">
                      <p className="c3-42d-decision-metric-label">在全部交易日期中出现概率</p>
                      <p className="c3-42d-decision-metric-value">{analysis.current_forecast.probability}%</p>
                    </div>
                  </div>
                </div>

                {/* 正收益和负收益部分 */}
                <div className="c3-42d-decision-returns-section">
                  <div className="c3-42d-decision-returns-column positive">
                    <h3 className="c3-42d-decision-returns-column-title">正收益</h3>
                    <div className="c3-42d-decision-returns-header">
                      <div className="c3-42d-decision-returns-bar positive-bar">
                        <span className="c3-42d-decision-returns-value">{analysis.positive_returns.final_positive_returns_percentage}%</span>
                        <span className="c3-42d-decision-returns-label">最终正收益占比</span>
                      </div>
                      <div className="c3-42d-decision-returns-bar average-bar">
                        <span className="c3-42d-decision-returns-value">{analysis.positive_returns.final_positive_returns_average}%</span>
                        <span className="c3-42d-decision-returns-label">最终正收益平均值</span>
                      </div>
                    </div>

                    <div className="c3-42d-decision-returns-card">
                      <p className="c3-42d-decision-returns-card-title">分布情况</p>
                      <div className="c3-42d-decision-returns-table">
                        <div className="c3-42d-decision-returns-table-row">
                          <span className="c3-42d-decision-returns-table-label">正收益比例0~15%</span>
                          <span className="c3-42d-decision-returns-table-value">{analysis.positive_returns.distribution['0-15%']}%</span>
                        </div>
                        <div className="c3-42d-decision-returns-table-row">
                          <span className="c3-42d-decision-returns-table-label">正收益比例15.01~30%</span>
                          <span className="c3-42d-decision-returns-table-value">{analysis.positive_returns.distribution['15.01-30%']}%</span>
                        </div>
                        <div className="c3-42d-decision-returns-table-row">
                          <span className="c3-42d-decision-returns-table-label">正收益比例30~60%</span>
                          <span className="c3-42d-decision-returns-table-value">{analysis.positive_returns.distribution['30-60%']}%</span>
                        </div>
                        <div className="c3-42d-decision-returns-table-row">
                          <span className="c3-42d-decision-returns-table-label">正收益比例大于60%</span>
                          <span className="c3-42d-decision-returns-table-value">{analysis.positive_returns.distribution['>60%']}%</span>
                        </div>
                      </div>
                    </div>

                    <div className="c3-42d-decision-returns-card">
                      <p className="c3-42d-decision-returns-card-title">收益统计</p>
                      <div className="c3-42d-decision-returns-table">
                        <div className="c3-42d-decision-returns-table-row">
                          <span className="c3-42d-decision-returns-table-label">最大正收益平均值</span>
                          <span className="c3-42d-decision-returns-table-value">
                            {correctedStats?.max_positive_returns_average || analysis.positive_returns.statistics.max_positive_returns_average}%
                          </span>
                        </div>
                        <div className="c3-42d-decision-returns-table-row">
                          <span className="c3-42d-decision-returns-table-label">最大正收益最大值</span>
                          <span className="c3-42d-decision-returns-table-value">
                            {correctedStats?.max_positive_returns_maximum || analysis.positive_returns.statistics.max_positive_returns_maximum}%
                          </span>
                        </div>
                        <div className="c3-42d-decision-returns-table-row">
                          <span className="c3-42d-decision-returns-table-label">最大正收益出现时间平均值</span>
                          <span className="c3-42d-decision-returns-table-value">
                    ...
                    <div className="c3-42d-decision-returns-card">
                      <p className="c3-42d-decision-returns-card-title">最大正收益平均出现天数</p>
                      <div className="c3-42d-decision-returns-table">
                        <div className="c3-42d-decision-returns-table-row">
                          <span className="c3-42d-decision-returns-table-label">0~14天内</span>
                          <span className="c3-42d-decision-returns-table-value">
                            {correctedStats?.timing_distribution['0-14_days'] || analysis.positive_returns.timing_distribution['0-14_days']}%
                          </span>
                        </div>
                        <div className="c3-42d-decision-returns-table-row">
                          <span className="c3-42d-decision-returns-table-label">15~28天内</span>
                          <span className="c3-42d-decision-returns-table-value">
                            {correctedStats?.timing_distribution['15-28_days'] || analysis.positive_returns.timing_distribution['15-28_days']}%
                          </span>
                        </div>
                        <div className="c3-42d-decision-returns-table-row">
                          <span className="c3-42d-decision-returns-table-label">29~42天内</span>
                          <span className="c3-42d-decision-returns-table-value">
                  ...
                  {/* 负收益部分 */}
                  <div className="c3-42d-decision-returns-column negative">
                    <h3 className="c3-42d-decision-returns-column-title">负收益</h3>
                    <div className="c3-42d-decision-returns-header">
                      <div className="c3-42d-decision-returns-bar negative-bar">
                        <span className="c3-42d-decision-returns-value">{analysis.negative_returns.final_negative_returns_percentage}%</span>
                        <span className="c3-42d-decision-returns-label">最终负收益比例</span>
                      </div>
                      <div className="c3-42d-decision-returns-bar average-bar">
                        <span className="c3-42d-decision-returns-value">{analysis.negative_returns.final_negative_returns_average}%</span>
                        <span className="c3-42d-decision-returns-label">最终负收益平均值</span>
                      </div>
                    </div>
                    ...
                    <div className="c3-42d-decision-returns-card">
                      ...
                      <div className="c3-42d-decision-returns-table">
                        <div className="c3-42d-decision-returns-table-row">
                          <span className="c3-42d-decision-returns-table-label">负收益比例0~15%</span>
                          <span className="c3-42d-decision-returns-table-value">{analysis.negative_returns.distribution['0-15%']}%</span>
                        </div>
                        <div className="c3-42d-decision-returns-table-row">
                          <span className="c3-42d-decision-returns-table-label">负收益比例15.01~30%</span>
                          <span className="c3-42d-decision-returns-table-value">{analysis.negative_returns.distribution['15.01-30%']}%</span>
                        </div>
                        <div className="c3-42d-decision-returns-table-row">
                          <span className="c3-42d-decision-returns-table-label">负收益比例30~60%</span>
                          <span className="c3-42d-decision-returns-table-value">{analysis.negative_returns.distribution['30-60%']}%</span>
                        </div>
                        <div className="c3-42d-decision-returns-table-row">
                          <span className="c3-42d-decision-returns-table-label">负收益比例小于60%</span>
                          <span className="c3-42d-decision-returns-table-value">{analysis.negative_returns.distribution['<60%']}%</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="c3-42d-decision-returns-card">
                      ...
                      <div className="c3-42d-decision-returns-table">
                        <div className="c3-42d-decision-returns-table-row">
                          <span className="c3-42d-decision-returns-table-label">最小负收益平均值</span>
                          <span className="c3-42d-decision-returns-table-value">
                            {correctedStats?.min_negative_returns_average || analysis.negative_returns.statistics.min_negative_returns_average}%
                          </span>
                        </div>
                        <div className="c3-42d-decision-returns-table-row">
                          <span className="c3-42d-decision-returns-table-label">最小负收益最小值</span>
                          <span className="c3-42d-decision-returns-table-value">
                ...
                {/* C3盈亏比部分 */}
                {analysis.c3_profit_loss_ratio && (
                  <div className="c3-42d-decision-profit-loss-section">
                    <p className="c3-42d-decision-profit-loss-title">C3盈亏比</p>
                    <div className="c3-42d-decision-profit-loss-summary">
                      <div className="c3-42d-decision-profit-loss-card">
                        <p className="c3-42d-decision-profit-loss-label">日期</p>
                        <p className="c3-42d-decision-profit-loss-value">{analysis.c3_profit_loss_ratio.date}</p>
                      </div>
                      <div className="c3-42d-decision-profit-loss-card">
                        <p className="c3-42d-decision-profit-loss-label">当前价格/元每吨</p>
                        <p className="c3-42d-decision-profit-loss-value">{analysis.c3_profit_loss_ratio.current_price.toLocaleString()}</p>
                      </div>
                      <div className="c3-42d-decision-profit-loss-card">
                        <p className="c3-42d-decision-profit-loss-label">评估价格/元每吨</p>
                        <p className="c3-42d-decision-profit-loss-value">{analysis.c3_profit_loss_ratio.evaluated_price.toLocaleString()}</p>
                      </div>
                      <div className="c3-42d-decision-profit-loss-card">
                        <p className="c3-42d-decision-profit-loss-label">价差比</p>
                        <p className="c3-42d-decision-profit-loss-value">{analysis.c3_profit_loss_ratio.price_difference_ratio}</p>
                      </div>
                    </div>
                  </div>
                )}
                ...
                {/* P5TC六周后预测模型评价 */}
                <div className="c3-42d-decision-model-section">
                  <p className="c3-42d-decision-model-title">P5TC六周后预测模型评价</p>
                  <div className="c3-42d-decision-model-summary">
                    <div className="c3-42d-decision-model-summary-card">
                      <p className="c3-42d-decision-model-summary-label">日期</p>
                      <p className="c3-42d-decision-model-summary-value">{analysis.model_evaluation.date}</p>
                    </div>
                    <div className="c3-42d-decision-model-summary-card">
                      <p className="c3-42d-decision-model-summary-label">当前价格/元每吨</p>
                      <p className="c3-42d-decision-model-summary-value">{analysis.model_evaluation.current_price.toLocaleString()}</p>
                    </div>
                    <div className="c3-42d-decision-model-summary-card">
                      <p className="c3-42d-decision-model-summary-label">预测42天后价差/元每吨</p>
                      <p className="c3-42d-decision-model-summary-value">{analysis.model_evaluation.forecast_42day_price_difference.toLocaleString()}</p>
                    </div>
                    <div className="c3-42d-decision-model-summary-card">
                      <p className="c3-42d-decision-model-summary-label">预测42天后价格/元每吨</p>
                      <p className="c3-42d-decision-model-summary-value">{analysis.model_evaluation.forecast_42day_price.toLocaleString()}</p>
                    </div>
                    <div className="c3-42d-decision-model-summary-card">
                      <p className="c3-42d-decision-model-summary-label">价差比</p>
                      <p className="c3-42d-decision-model-summary-value">{analysis.model_evaluation.price_difference_ratio}</p>
                    </div>
                  </div>

                  {/* 评价表格 */}
                  <div className="c3-42d-decision-model-table">
                    <div className="c3-42d-decision-model-table-header">
                      <div className="c3-42d-decision-model-table-header-cell">区间</div>
                      <div className="c3-42d-decision-model-table-header-cell">历史判断正确率</div>
                      <div className="c3-42d-decision-model-table-header-cell">历史预测实际值/元每吨</div>
                      <div className="c3-42d-decision-model-table-header-cell">历史预测拟合值/元每吨</div>
                    </div>
                    {analysis.model_evaluation.evaluation_ranges.map((range, index) => (
                      <div key={index} className="c3-42d-decision-model-table-row">
                        <div className="c3-42d-decision-model-table-cell">{range.range}</div>
                        <div className="c3-42d-decision-model-table-cell">{range.historical_accuracy_rate.toFixed(2)}%</div>
                        <div className="c3-42d-decision-model-table-cell">{range.historical_actual_value.toLocaleString()}</div>
                        <div className="c3-42d-decision-model-table-cell">{range.historical_fit_value.toLocaleString()}</div>
                      </div>
                    ))}
                  </div>
                </div>
...
export default C3_42dDecisionPage
