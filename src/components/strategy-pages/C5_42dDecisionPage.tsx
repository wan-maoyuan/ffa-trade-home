...
const C5_42dDecisionPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchDecisionData = async () => {
      try {
        setLoading(true)
        setError(null)
        const response = await fetch('https://aqua.navgreen.cn/api/strategy/c5/decision')
        ...
        if (result.code === 200 && result.data.records && result.data.records.length > 0) {
          ...
          const c5Analysis = record.raw_data?.contracts?.c5_analysis
          ...
          if (rawTableData && Array.isArray(rawTableData)) {
            // 解析基础信息
            // Row 1: ["做多胜率统计","盈亏比：","14：1"]
            // Row 2: ["做多","2025-11-26","10.96","17%"]
            // Row 3: ["日期","当期值","综合价差比"]
            // Row 4: ["建议交易方向","15%-30%","12.79","13%"]
            // Row 5: ["综合价差比区间","2026-01-07预测值","在全部交易日期中出现概率"]
            
            let profitLossRatio = 14.0
            ...
            // Row 10: ["正收益比例0～15%","正收益比例15.01～30%","正收益比例30～60%","正收益比例大于60%"]
            // Row 11: ["30%","19%","32%","16%"]
            // Row 13: ["最大正收益平均值","最大正收益最大值","最大正收益出现时间平均值"]
            // Row 14: ["43%","85%","31"]
            // Row 16: ["0～14天内","15～28天内","29～42天内"]
            // Row 17: ["24%","27%","49%"]
            ...
            // Row 18: ["负收益"]
            // Row 19: ["29%","-6%"]
            // Row 20: ["最终负收益比例","最终负收益平均值"]
            // Row 22: ["负收益比例0～15%","负收益比15.01～30%","负收益比30～60%","负收益比小于60%"]
            // Row 23: ["97%","3%","0%","0%"]
            // Row 25: ["最小负收益平均值","最小负收益最小值"]
            // Row 26: ["-11%","-21%"]
            ...
            // 解析C5盈亏比数据
            // 查找"C5盈亏比"行
            // Row 28: ['C5盈亏比'] (在rawTableData中的实际索引)
            // Row 29: ['2025-11-26', '10.96', '12.07', '10%'] - 数据行（日期、当前价格、评估价格、价差比）
            // Row 30: ['日期', '当前价格/元每吨', '评估价格/元每吨', '价差比'] - 标题行
            // Row 31: ['0～0.15'] - 区间（可忽略）
            // Row 32: ['72%', '12%', '28%', '8%'] - 数据行（42天后盈利比例、收益均值、42天后亏损比例、亏损均值）
            // Row 33: ['42天后盈利比例', '收益均值', '42天后亏损比例', '亏损均值/元每吨'] - 标题行
            // Row 34: ['最大收益时间在各时间段的出现概率'] - 标题
            // Row 35: ['0～14天', '15天～28天', '29天～42天'] - 标题行
            // Row 36: ['33%', '33%', '33%'] - 最大收益时间数据
            // Row 37: ['9%', '28%'] - 最大风险均值、最大风险极值
            // Row 38: ['最大风险均值/元每吨', '最大风险极值/元每吨'] - 标题行
            // Row 39: ['最大风险时间在各时间段的出现概率'] - 标题
            // Row 40: ['0～14天', '15天～28天', '29天～42天'] - 标题行
            // Row 41: ['54%', '23%', '23%'] - 最大风险时间数据
            let c5ProfitLossData: C5ProfitLossRatio | null = null
            for (let i = 0; i < rawTableData.length; i++) {
              const row = rawTableData[i]
              if (Array.isArray(row) && row.length > 0 && row[0] === 'C5盈亏比') {
                // Row 2: 数据行（日期、当前价格、评估价格、价差比）
                if (i + 1 < rawTableData.length) {
                  const dataRow = rawTableData[i + 1]
                  if (Array.isArray(dataRow) && dataRow.length >= 4) {
          ...
          if (c5Analysis) {
            setAnalysis(c5Analysis)
          } else if (parsedData) {
            setAnalysis(parsedData)
          } else if (record.core_data) {
            // 如果c5_analysis不存在，使用core_data构造基本数据
  ...
  return (
    <div className="c5-42d-decision-panel">
      <SideMenu currentPage="strategy" />
      
      <div className="c5-42d-decision-container">
        {/* 返回按钮 */}
        <button 
          className="c5-42d-decision-back-button"
          onClick={handleBackClick}
          type="button"
          aria-label="返回上一级"
        >
          <span className="c5-42d-decision-back-icon">←</span>
          <span className="c5-42d-decision-back-text">返回</span>
        </button>

        <div className="c5-42d-decision-page">
          {/* 页面标题 */}
          <p className="c5-42d-decision-page-title">C5现货应用决策（42天后）</p>

          {loading ? (
            <div className="c5-42d-decision-loading">
              <div className="c5-42d-decision-loading-spinner"></div>
              <p>加载中...</p>
            </div>
          ) : error ? (
            <div className="c5-42d-decision-error">
              <p>{error}</p>
            </div>
          ) : analysis ? (
            <>
              {/* 做多胜率统计 */}
              <div className="c5-42d-decision-header">
                <div className="c5-42d-decision-chips">
                  <button type="button" className="c5-42d-decision-chip">做多胜率统计</button>
                  <button type="button" className="c5-42d-decision-chip">
              ...
              {/* 主要内容区域 */}
              <div className="c5-42d-decision-content">
                {/* 左侧策略卡片和右侧指标网格 */}
                <div className="c5-42d-decision-cards-container">
                  {/* 左侧策略卡片 */}
                  <div className="c5-42d-decision-card-left">
                    <span className="c5-42d-decision-card-badge">多头策略</span>
                    <div className="c5-42d-decision-card-glow" />
                    <div className="c5-42d-decision-card-overlay">
                      <div className="c5-42d-decision-card-title">
                        {analysis.trading_recommendation.recommended_direction}
                      </div>
                      <div className="c5-42d-decision-card-desc">建议交易方向</div>
                    </div>
                  </div>

                  {/* 右侧指标网格 */}
                  <div className="c5-42d-decision-metrics-grid">
                    <div className="c5-42d-decision-metric-card">
                      <p className="c5-42d-decision-metric-label">日期</p>
                      <p className="c5-42d-decision-metric-value">{analysis.current_forecast.date}</p>
                    </div>
                    <div className="c5-42d-decision-metric-card">
                      <p className="c5-42d-decision-metric-label">当期值</p>
                      <p className="c5-42d-decision-metric-value">
                        {analysis.current_forecast.current_value.toLocaleString()}
                      </p>
                    </div>
                    <div className="c5-42d-decision-metric-card">
                      <p className="c5-42d-decision-metric-label">综合价差比</p>
                      <p className="c5-42d-decision-metric-value">{analysis.current_forecast.overall_price_difference_ratio}</p>
                    </div>
                    <div className="c5-42d-decision-metric-card">
                      <p className="c5-42d-decision-metric-label">综合价差比区间</p>
                      <p className="c5-42d-decision-metric-value">{analysis.current_forecast.overall_price_difference_range}</p>
                    </div>
                    <div className="c5-42d-decision-metric-card">
                      <p className="c5-42d-decision-metric-label">2026-01-06预测值</p>
                      <p className="c5-42d-decision-metric-value">
                        {analysis.current_forecast.forecast_value.toLocaleString()}
                      </p>
                    </div>
                    <div className="c5-42d-decision-metric-card">
                      <p className="c5-42d-decision-metric-label">在全部交易日期中出现概率</p>
                      <p className="c5-42d-decision-metric-value">{analysis.current_forecast.probability}%</p>
                    </div>
                  </div>
                </div>

                {/* 正收益和负收益部分 */}
                <div className="c5-42d-decision-returns-section">
                  <div className="c5-42d-decision-returns-column positive">
                    <h3 className="c5-42d-decision-returns-column-title">正收益</h3>
                    <div className="c5-42d-decision-returns-header">
                      <div className="c5-42d-decision-returns-bar positive-bar">
                        <span className="c5-42d-decision-returns-value">{analysis.positive_returns.final_positive_returns_percentage}%</span>
                        <span className="c5-42d-decision-returns-label">最终正收益占比</span>
                      </div>
                      <div className="c5-42d-decision-returns-bar average-bar">
                        <span className="c5-42d-decision-returns-value">{analysis.positive_returns.final_positive_returns_average}%</span>
                        <span className="c5-42d-decision-returns-label">最终正收益平均值</span>
                      </div>
                    </div>

                    <div className="c5-42d-decision-returns-card">
                      <p className="c5-42d-decision-returns-card-title">分布情况</p>
                      <div className="c5-42d-decision-returns-table">
                        <div className="c5-42d-decision-returns-table-row">
                          <span className="c5-42d-decision-returns-table-label">正收益比例0~15%</span>
                          <span className="c5-42d-decision-returns-table-value">{analysis.positive_returns.distribution['0-15%']}%</span>
                        </div>
                        <div className="c5-42d-decision-returns-table-row">
                          <span className="c5-42d-decision-returns-table-label">正收益比例15.01~30%</span>
                          <span className="c5-42d-decision-returns-table-value">{analysis.positive_returns.distribution['15.01-30%']}%</span>
                        </div>
                        <div className="c5-42d-decision-returns-table-row">
                          <span className="c5-42d-decision-returns-table-label">正收益比例30~60%</span>
                          <span className="c5-42d-decision-returns-table-value">{analysis.positive_returns.distribution['30-60%']}%</span>
                        </div>
                        <div className="c5-42d-decision-returns-table-row">
                          <span className="c5-42d-decision-returns-table-label">正收益比例大于60%</span>
                          <span className="c5-42d-decision-returns-table-value">{analysis.positive_returns.distribution['>60%']}%</span>
                        </div>
                      </div>
                    </div>

                    <div className="c5-42d-decision-returns-card">
                      <p className="c5-42d-decision-returns-card-title">收益统计</p>
                      <div className="c5-42d-decision-returns-table">
                        <div className="c5-42d-decision-returns-table-row">
                          <span className="c5-42d-decision-returns-table-label">最大正收益平均值</span>
                          <span className="c5-42d-decision-returns-table-value">
                            {correctedStats?.max_positive_returns_average || analysis.positive_returns.statistics.max_positive_returns_average}%
                          </span>
                        </div>
                        <div className="c5-42d-decision-returns-table-row">
                          <span className="c5-42d-decision-returns-table-label">最大正收益最大值</span>
                          <span className="c5-42d-decision-returns-table-value">
                            {correctedStats?.max_positive_returns_maximum || analysis.positive_returns.statistics.max_positive_returns_maximum}%
                          </span>
                        </div>
                        <div className="c5-42d-decision-returns-table-row">
                          <span className="c5-42d-decision-returns-table-label">最大正收益出现时间平均值</span>
                          <span className="c5-42d-decision-returns-table-value">
                    ...
                    <div className="c5-42d-decision-returns-card">
                      <p className="c5-42d-decision-returns-card-title">最大正收益平均出现天数</p>
                      <div className="c5-42d-decision-returns-table">
                        <div className="c5-42d-decision-returns-table-row">
                          <span className="c5-42d-decision-returns-table-label">0~14天内</span>
                          <span className="c5-42d-decision-returns-table-value">
                            {correctedStats?.timing_distribution['0-14_days'] || analysis.positive_returns.timing_distribution['0-14_days']}%
                          </span>
                        </div>
                        <div className="c5-42d-decision-returns-table-row">
                          <span className="c5-42d-decision-returns-table-label">15~28天内</span>
                          <span className="c5-42d-decision-returns-table-value">
                            {correctedStats?.timing_distribution['15-28_days'] || analysis.positive_returns.timing_distribution['15-28_days']}%
                          </span>
                        </div>
                        <div className="c5-42d-decision-returns-table-row">
                          <span className="c5-42d-decision-returns-table-label">29~42天内</span>
                          <span className="c5-42d-decision-returns-table-value">
                  ...
                  {/* 负收益部分 */}
                  <div className="c5-42d-decision-returns-column negative">
                    <h3 className="c5-42d-decision-returns-column-title">负收益</h3>
                    <div className="c5-42d-decision-returns-header">
                      <div className="c5-42d-decision-returns-bar negative-bar">
                        <span className="c5-42d-decision-returns-value">{analysis.negative_returns.final_negative_returns_percentage}%</span>
                        <span className="c5-42d-decision-returns-label">最终负收益比例</span>
                      </div>
                      <div className="c5-42d-decision-returns-bar average-bar">
                        <span className="c5-42d-decision-returns-value">{analysis.negative_returns.final_negative_returns_average}%</span>
                        <span className="c5-42d-decision-returns-label">最终负收益平均值</span>
                      </div>
                    </div>
                    ...
                    <div className="c5-42d-decision-returns-card">
                      ...
                      <div className="c5-42d-decision-returns-table">
                        <div className="c5-42d-decision-returns-table-row">
                          <span className="c5-42d-decision-returns-table-label">负收益比例0~15%</span>
                          <span className="c5-42d-decision-returns-table-value">{analysis.negative_returns.distribution['0-15%']}%</span>
                        </div>
                        <div className="c5-42d-decision-returns-table-row">
                          <span className="c5-42d-decision-returns-table-label">负收益比例15.01~30%</span>
                          <span className="c5-42d-decision-returns-table-value">{analysis.negative_returns.distribution['15.01-30%']}%</span>
                        </div>
                        <div className="c5-42d-decision-returns-table-row">
                          <span className="c5-42d-decision-returns-table-label">负收益比例30~60%</span>
                          <span className="c5-42d-decision-returns-table-value">{analysis.negative_returns.distribution['30-60%']}%</span>
                        </div>
                        <div className="c5-42d-decision-returns-table-row">
                          <span className="c5-42d-decision-returns-table-label">负收益比例小于60%</span>
                          <span className="c5-42d-decision-returns-table-value">{analysis.negative_returns.distribution['<60%']}%</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="c5-42d-decision-returns-card">
                      ...
                      <div className="c5-42d-decision-returns-table">
                        <div className="c5-42d-decision-returns-table-row">
                          <span className="c5-42d-decision-returns-table-label">最小负收益平均值</span>
                          <span className="c5-42d-decision-returns-table-value">
                            {correctedStats?.min_negative_returns_average || analysis.negative_returns.statistics.min_negative_returns_average}%
                          </span>
                        </div>
                        <div className="c5-42d-decision-returns-table-row">
                          <span className="c5-42d-decision-returns-table-label">最小负收益最小值</span>
                          <span className="c5-42d-decision-returns-table-value">
                ...
                {/* C5盈亏比部分 */}
                {analysis.c5_profit_loss_ratio && (
                  <div className="c5-42d-decision-profit-loss-section">
                    <p className="c5-42d-decision-profit-loss-title">C5盈亏比</p>
                    <div className="c5-42d-decision-profit-loss-summary">
                      <div className="c5-42d-decision-profit-loss-card">
                        <p className="c5-42d-decision-profit-loss-label">日期</p>
                        <p className="c5-42d-decision-profit-loss-value">{analysis.c5_profit_loss_ratio.date}</p>
                      </div>
                      <div className="c5-42d-decision-profit-loss-card">
                        <p className="c5-42d-decision-profit-loss-label">当前价格/元每吨</p>
                        <p className="c5-42d-decision-profit-loss-value">{analysis.c5_profit_loss_ratio.current_price.toLocaleString()}</p>
                      </div>
                      <div className="c5-42d-decision-profit-loss-card">
                        <p className="c5-42d-decision-profit-loss-label">评估价格/元每吨</p>
                        <p className="c5-42d-decision-profit-loss-value">{analysis.c5_profit_loss_ratio.evaluated_price.toLocaleString()}</p>
                      </div>
                      <div className="c5-42d-decision-profit-loss-card">
                        <p className="c5-42d-decision-profit-loss-label">价差比</p>
                        <p className="c5-42d-decision-profit-loss-value">{analysis.c5_profit_loss_ratio.price_difference_ratio}</p>
                      </div>
                      <div className="c5-42d-decision-profit-loss-card">
                        <p className="c5-42d-decision-profit-loss-label">42天后盈利比例</p>
                        <p className="c5-42d-decision-profit-loss-value">{analysis.c5_profit_loss_ratio.profitability_ratio_after_42days}%</p>
                      </div>
                      <div className="c5-42d-decision-profit-loss-card">
                        <p className="c5-42d-decision-profit-loss-label">收益均值</p>
                        <p className="c5-42d-decision-profit-loss-value">{analysis.c5_profit_loss_ratio.average_returns}%</p>
                      </div>
                      <div className="c5-42d-decision-profit-loss-card">
                        <p className="c5-42d-decision-profit-loss-label">42天后亏损比例</p>
                        <p className="c5-42d-decision-profit-loss-value">{analysis.c5_profit_loss_ratio.loss_ratio_after_42days}%</p>
                      </div>
                      <div className="c5-42d-decision-profit-loss-card">
                        <p className="c5-42d-decision-profit-loss-label">亏损均值/元每吨</p>
                        <p className="c5-42d-decision-profit-loss-value">{analysis.c5_profit_loss_ratio.average_loss}%</p>
                      </div>
                    </div>

                    <div className="c5-42d-decision-profit-loss-timing">
                      <div className="c5-42d-decision-profit-loss-timing-card">
                        <p className="c5-42d-decision-profit-loss-timing-title">最大收益时间在各时间段的出现概率</p>
                        <div className="c5-42d-decision-profit-loss-timing-grid">
                          <div className="c5-42d-decision-profit-loss-timing-item">
                            <span className="c5-42d-decision-profit-loss-timing-label">0~14天</span>
                            <span className="c5-42d-decision-profit-loss-timing-value">{analysis.c5_profit_loss_ratio.max_returns_timing_distribution['0-14_days']}%</span>
                          </div>
                          <div className="c5-42d-decision-profit-loss-timing-item">
                            <span className="c5-42d-decision-profit-loss-timing-label">15天~28天</span>
                            <span className="c5-42d-decision-profit-loss-timing-value">{analysis.c5_profit_loss_ratio.max_returns_timing_distribution['15-28_days']}%</span>
                          </div>
                          <div className="c5-42d-decision-profit-loss-timing-item">
                            <span className="c5-42d-decision-profit-loss-timing-label">29天~42天</span>
                            <span className="c5-42d-decision-profit-loss-timing-value">{analysis.c5_profit_loss_ratio.max_returns_timing_distribution['29-42_days']}%</span>
                          </div>
                        </div>
                      </div>
                      <div className="c5-42d-decision-profit-loss-timing-card">
                        <p className="c5-42d-decision-profit-loss-timing-title">最大风险时间在各时间段的出现概率</p>
                        <div className="c5-42d-decision-profit-loss-timing-grid">
                          <div className="c5-42d-decision-profit-loss-timing-item">
                            <span className="c5-42d-decision-profit-loss-timing-label">0~14天</span>
                            <span className="c5-42d-decision-profit-loss-timing-value">{analysis.c5_profit_loss_ratio.max_risk_timing_distribution['0-14_days']}%</span>
                          </div>
                          <div className="c5-42d-decision-profit-loss-timing-item">
                            <span className="c5-42d-decision-profit-loss-timing-label">15天~28天</span>
                            <span className="c5-42d-decision-profit-loss-timing-value">{analysis.c5_profit_loss_ratio.max_risk_timing_distribution['15-28_days']}%</span>
                          </div>
                          <div className="c5-42d-decision-profit-loss-timing-item">
                            <span className="c5-42d-decision-profit-loss-timing-label">29天~42天</span>
                            <span className="c5-42d-decision-profit-loss-timing-value">{analysis.c5_profit_loss_ratio.max_risk_timing_distribution['29-42_days']}%</span>
                          </div>
                        </div>
                      </div>
                    </div>

                    <div className="c5-42d-decision-profit-loss-risk">
                      <div className="c5-42d-decision-profit-loss-risk-card">
                        <p className="c5-42d-decision-profit-loss-risk-label">最大风险均值/元每吨</p>
                        <p className="c5-42d-decision-profit-loss-risk-value">{analysis.c5_profit_loss_ratio.max_risk_average}%</p>
                      </div>
                      <div className="c5-42d-decision-profit-loss-risk-card">
                        <p className="c5-42d-decision-profit-loss-risk-label">最大风险极值/元每吨</p>
                        <p className="c5-42d-decision-profit-loss-risk-value">{analysis.c5_profit_loss_ratio.max_risk_extreme}%</p>
                      </div>
                    </div>
                  </div>
                )}
                ...
                {/* P5TC六周后预测模型评价 */}
                <div className="c5-42d-decision-model-section">
                  <p className="c5-42d-decision-model-title">P5TC六周后预测模型评价</p>
                  <div className="c5-42d-decision-model-summary">
                    <div className="c5-42d-decision-model-summary-card">
                      <p className="c5-42d-decision-model-summary-label">日期</p>
                      <p className="c5-42d-decision-model-summary-value">{analysis.model_evaluation.date}</p>
                    </div>
                    <div className="c5-42d-decision-model-summary-card">
                      <p className="c5-42d-decision-model-summary-label">当前价格/元每吨</p>
                      <p className="c5-42d-decision-model-summary-value">{analysis.model_evaluation.current_price.toLocaleString()}</p>
                    </div>
                    <div className="c5-42d-decision-model-summary-card">
                      <p className="c5-42d-decision-model-summary-label">预测42天后价差/元每吨</p>
                      <p className="c5-42d-decision-model-summary-value">{analysis.model_evaluation.forecast_42day_price_difference.toLocaleString()}</p>
                    </div>
                    <div className="c5-42d-decision-model-summary-card">
                      <p className="c5-42d-decision-model-summary-label">预测42天后价格/元每吨</p>
                      <p className="c5-42d-decision-model-summary-value">{analysis.model_evaluation.forecast_42day_price.toLocaleString()}</p>
                    </div>
                    <div className="c5-42d-decision-model-summary-card">
                      <p className="c5-42d-decision-model-summary-label">价差比</p>
                      <p className="c5-42d-decision-model-summary-value">{analysis.model_evaluation.price_difference_ratio}</p>
                    </div>
                  </div>

                  {/* 评价表格 */}
                  <div className="c5-42d-decision-model-table">
                    <div className="c5-42d-decision-model-table-header">
                      <div className="c5-42d-decision-model-table-header-cell">区间</div>
                      <div className="c5-42d-decision-model-table-header-cell">历史判断正确率</div>
                      <div className="c5-42d-decision-model-table-header-cell">历史预测实际值/元每吨</div>
                      <div className="c5-42d-decision-model-table-header-cell">历史预测拟合值/元每吨</div>
                    </div>
                    {analysis.model_evaluation.evaluation_ranges.map((range, index) => (
                      <div key={index} className="c5-42d-decision-model-table-row">
                        <div className="c5-42d-decision-model-table-cell">{range.range}</div>
                        <div className="c5-42d-decision-model-table-cell">{range.historical_accuracy_rate.toFixed(2)}%</div>
                        <div className="c5-42d-decision-model-table-cell">{range.historical_actual_value.toLocaleString()}</div>
                        <div className="c5-42d-decision-model-table-cell">{range.historical_fit_value.toLocaleString()}</div>
                      </div>
                    ))}
                  </div>
                </div>
