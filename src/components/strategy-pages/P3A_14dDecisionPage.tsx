...
import './P3A_14dDecisionPage.css'
...
interface P3ACurrentEvaluation {
  date: string
...
interface P3A_14dAnalysis {
  trading_recommendation: TradingRecommendation
  current_forecast: CurrentForecast
  positive_returns: PositiveReturns
  negative_returns: NegativeReturns
  p3a_current_evaluation: P3ACurrentEvaluation
...
interface ApiResponse {
  ...
  data: {
    ...
    records: Array<{
      ...
      raw_data?: {
        contracts?: {
          p3a_14d_analysis?: P3A_14dAnalysis
        }
      }
    }>
  }
...
const P3A_14dDecisionPage: React.FC = () => {
  const navigate = useNavigate()
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [analysis, setAnalysis] = useState<P3A_14dAnalysis | null>(null)
  ...
  useEffect(() => {
    const fetchDecisionData = async () => {
      try {
        setLoading(true)
        setError(null)
        const response = await fetch('https://aqua.navgreen.cn/api/strategy/p3a_14d/decision')
        ...
        if (result.code === 200 && result.data.records && result.data.records.length > 0) {
          ...
          if (rawTableData && Array.isArray(rawTableData)) {
            ...
            let p3aCurrentEvalData: P3ACurrentEvaluation | null = null
            
            for (let i = 0; i < rawTableData.length; i++) {
              const row = rawTableData[i]
              if (Array.isArray(row) && row.length > 0 && row[0] === 'P3A当前评估价格') {
            ...
            parsedData = {
              ...
              p3a_current_evaluation: p3aCurrentEvalData || {
                date: date || result.data.date,
  ...
  return (
    <div className="p3a-14d-decision-panel">
      <SideMenu currentPage="strategy" />
      
      <div className="p3a-14d-decision-container">
        {/* 返回按钮 */}
        <button 
          className="p3a-14d-decision-back-button"
          onClick={handleBackClick}
          type="button"
          aria-label="返回上一级"
        >
          <span className="p3a-14d-decision-back-icon">←</span>
          <span className="p3a-14d-decision-back-text">返回</span>
        </button>

        <div className="p3a-14d-decision-page">
          {/* 页面标题 */}
          <p className="p3a-14d-decision-page-title">P3A现货应用决策（14天后）</p>

          {loading ? (
            <div className="p3a-14d-decision-loading">
              <div className="p3a-14d-decision-loading-spinner"></div>
              <p>加载中...</p>
            </div>
          ) : error ? (
            <div className="p3a-14d-decision-error">
              <p>{error}</p>
            </div>
          ) : analysis ? (
            <>
              {/* 做空胜率统计 */}
              <div className="p3a-14d-decision-header">
                <div className="p3a-14d-decision-chips">
                  <button type="button" className="p3a-14d-decision-chip">做空胜率统计</button>
                  <button type="button" className="p3a-14d-decision-chip">
              ...
              {/* 主要内容区域 */}
              <div className="p3a-14d-decision-content">
                {/* 左侧策略卡片和右侧指标网格 */}
                <div className="p3a-14d-decision-cards-container">
                  {/* 左侧策略卡片 */}
                  <div className="p3a-14d-decision-card-left">
                    <span className="p3a-14d-decision-card-badge">空头策略</span>
                    <div className="p3a-14d-decision-card-glow" />
                    <div className="p3a-14d-decision-card-overlay">
                      <div className="p3a-14d-decision-card-title">
                        {analysis.trading_recommendation.recommended_direction}
                      </div>
                      <div className="p3a-14d-decision-card-desc">建议交易方向</div>
                    </div>
                  </div>

                  {/* 右侧指标网格 */}
                  <div className="p3a-14d-decision-metrics-grid">
                    <div className="p3a-14d-decision-metric-card">
                      <p className="p3a-14d-decision-metric-label">日期</p>
                      <p className="p3a-14d-decision-metric-value">{analysis.current_forecast.date}</p>
                    </div>
                    <div className="p3a-14d-decision-metric-card">
                      <p className="p3a-14d-decision-metric-label">当期值</p>
                      <p className="p3a-14d-decision-metric-value">
                        {analysis.current_forecast.current_value.toLocaleString()}
                      </p>
                    </div>
                    <div className="p3a-14d-decision-metric-card">
                      <p className="p3a-14d-decision-metric-label">综合价差比</p>
                      <p className="p3a-14d-decision-metric-value">{analysis.current_forecast.overall_price_difference_ratio}</p>
                    </div>
                    <div className="p3a-14d-decision-metric-card">
                      <p className="p3a-14d-decision-metric-label">档位区间</p>
                      <p className="p3a-14d-decision-metric-value">{analysis.current_forecast.gear_interval}</p>
                    </div>
                    <div className="p3a-14d-decision-metric-card">
                      <p className="p3a-14d-decision-metric-label">
                    ...
                    <div className="p3a-14d-decision-metric-card">
                      <p className="p3a-14d-decision-metric-label">在全部交易日期中出现概率</p>
                      <p className="p3a-14d-decision-metric-value">{analysis.current_forecast.probability}%</p>
                    </div>
                  </div>
                </div>

                {/* 正收益和负收益部分 */}
                <div className="p3a-14d-decision-returns-section">
                  <div className="p3a-14d-decision-returns-column positive">
                    <h3 className="p3a-14d-decision-returns-column-title">正收益</h3>
                    <div className="p3a-14d-decision-returns-header">
                      <div className="p3a-14d-decision-returns-bar positive-bar">
                        <span className="p3a-14d-decision-returns-value">{analysis.positive_returns.final_positive_returns_percentage}%</span>
                        <span className="p3a-14d-decision-returns-label">最终正收益占比</span>
                      </div>
                      <div className="p3a-14d-decision-returns-bar average-bar">
                        <span className="p3a-14d-decision-returns-value">{analysis.positive_returns.final_positive_returns_average.toLocaleString()}</span>
                        <span className="p3a-14d-decision-returns-label">最终正收益平均值</span>
                      </div>
                    </div>
                  </div>

                  {/* 负收益部分 */}
                  <div className="p3a-14d-decision-returns-column negative">
                    <h3 className="p3a-14d-decision-returns-column-title">负收益</h3>
                    <div className="p3a-14d-decision-returns-header">
                      <div className="p3a-14d-decision-returns-bar negative-bar">
                        <span className="p3a-14d-decision-returns-value">{analysis.negative_returns.final_negative_returns_percentage}%</span>
                        <span className="p3a-14d-decision-returns-label">最终负收益比例</span>
                      </div>
                      <div className="p3a-14d-decision-returns-bar average-bar">
                        <span className="p3a-14d-decision-returns-value">{analysis.negative_returns.final_negative_returns_average.toLocaleString()}</span>
                        <span className="p3a-14d-decision-returns-label">最终负收益平均值</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                {/* P3A当前评估价格部分 */}
                {analysis.p3a_current_evaluation && (
                  <div className="p3a-14d-decision-evaluation-section">
                    <p className="p3a-14d-decision-evaluation-title">P3A当前评估价格</p>
                    <div className="p3a-14d-decision-evaluation-summary">
                      <div className="p3a-14d-decision-evaluation-card">
                        <p className="p3a-14d-decision-evaluation-label">日期</p>
                        <p className="p3a-14d-decision-evaluation-value">{analysis.p3a_current_evaluation.date}</p>
                      </div>
                      <div className="p3a-14d-decision-evaluation-card">
                        <p className="p3a-14d-decision-evaluation-label">当前价格/元每吨</p>
                        <p className="p3a-14d-decision-evaluation-value">{analysis.p3a_current_evaluation.current_price.toLocaleString()}</p>
                      </div>
                      <div className="p3a-14d-decision-evaluation-card">
                        <p className="p3a-14d-decision-evaluation-label">评估价格/元每吨</p>
                        <p className="p3a-14d-decision-evaluation-value">{analysis.p3a_current_evaluation.evaluated_price.toLocaleString()}</p>
                      </div>
                      <div className="p3a-14d-decision-evaluation-card">
                        <p className="p3a-14d-decision-evaluation-label">价差比</p>
                        <p className="p3a-14d-decision-evaluation-value">{analysis.p3a_current_evaluation.price_difference_ratio}</p>
                      </div>
                    </div>
                  </div>
                )}
                ...
                {/* P5TC二周后预测模型评价 */}
                <div className="p3a-14d-decision-model-section">
                  <p className="p3a-14d-decision-model-title">P5TC二周后预测模型评价</p>
                  <div className="p3a-14d-decision-model-summary">
                    <div className="p3a-14d-decision-model-summary-card">
                      <p className="p3a-14d-decision-model-summary-label">日期</p>
                      <p className="p3a-14d-decision-model-summary-value">{analysis.model_evaluation.date}</p>
                    </div>
                    <div className="p3a-14d-decision-model-summary-card">
                      <p className="p3a-14d-decision-model-summary-label">当前价格/元每吨</p>
                      <p className="p3a-14d-decision-model-summary-value">{analysis.model_evaluation.current_price.toLocaleString()}</p>
                    </div>
                    <div className="p3a-14d-decision-model-summary-card">
                      <p className="p3a-14d-decision-model-summary-label">预测14天后价差/元每吨</p>
                      <p className="p3a-14d-decision-model-summary-value">{analysis.model_evaluation.forecast_14day_price_difference.toLocaleString()}</p>
                    </div>
                    <div className="p3a-14d-decision-model-summary-card">
                      <p className="p3a-14d-decision-model-summary-label">预测14天后价格/元每吨</p>
                      <p className="p3a-14d-decision-model-summary-value">{analysis.model_evaluation.forecast_14day_price.toLocaleString()}</p>
                    </div>
                    <div className="p3a-14d-decision-model-summary-card">
                      <p className="p3a-14d-decision-model-summary-label">价差比</p>
                      <p className="p3a-14d-decision-model-summary-value">{analysis.model_evaluation.price_difference_ratio}</p>
                    </div>
                  </div>

                  {/* 评价表格 */}
                  <div className="p3a-14d-decision-model-table">
                    <div className="p3a-14d-decision-model-table-header">
                      <div className="p3a-14d-decision-model-table-header-cell">区间</div>
                      <div className="p3a-14d-decision-model-table-header-cell">历史判断正确率</div>
                      <div className="p3a-14d-decision-model-table-header-cell">历史预测实际值/元每吨</div>
                      <div className="p3a-14d-decision-model-table-header-cell">历史预测拟合值/元每吨</div>
                    </div>
                    {analysis.model_evaluation.evaluation_ranges.map((range, index) => (
                      <div key={index} className="p3a-14d-decision-model-table-row">
                        <div className="p3a-14d-decision-model-table-cell">{range.range}</div>
                        <div className="p3a-14d-decision-model-table-cell">{range.historical_accuracy_rate.toFixed(2)}%</div>
                        <div className="p3a-14d-decision-model-table-cell">{range.historical_actual_value.toLocaleString()}</div>
                        <div className="p3a-14d-decision-model-table-cell">{range.historical_fit_value.toLocaleString()}</div>
                      </div>
                    ))}
                  </div>
                </div>
...
export default P3A_14dDecisionPage
